...
// -- ADDED BLE communication for Scratch AI Jorge Gonzales August 2020 https://ottoschool.com/scratch/
// -- ADDED LEDs Neo pixel control for funby Jorge Gonzales August 2020
//-------------------------------------------------------------------------
#include <EEPROM.h>
// NEOPIXEL LED PINs //////////////////////////////////////////////////////////////////////////////
...
void setup() {
  ...
  strip.begin();
  strip.setPixelColor(0, 0, 0, 64); // Led 0
  ...
  SCmd.addCommand("X", receiveShine);     //  sendAck & sendFinalAck
...
//-- Function to receive Shine command
void receiveShine() {
  //sendAck & stop if necessary
  sendAck();
  Otto.home();
  ...
  strip.setPixelColor(0, r, g, b); // Led 0
  strip.show();
  delay(300);
  sendFinalAck();
}
...
bool checkIfConnected(){
  ...
  if(state == HIGH){
    ...
    if(confirmedConnected == false){
      ...
      if(lastState == LOW){
        ...
        if(now - highStartTimestamp >= msToDefineConnection){
          ...
          Otto.sing(S_connection);
          strip.setPixelColor(0, 0, 64, 0); // Led 0
          strip.show();
          // Empty the buffer
    ...
    if(lastState == HIGH && confirmedConnected == true){
      ...
      strip.setPixelColor(0, 64, 0, 0); // Led 0
      strip.show();
